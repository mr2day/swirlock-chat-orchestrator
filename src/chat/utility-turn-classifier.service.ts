...
export class UtilityTurnClassifierService {
  ...
  private buildPrompt(input: ClassifyTurnInput): string {
    ...
    return [
      ...
      'The Utility LLM must never generate user-visible assistant text. Every assistant reply must be generated by the final-answer LLM.',
      '',
      'Allowed routes:',
      ...
      '- Casual greetings, social check-ins, acknowledgements, thanks, and goodbyes are final_answer turns with no retrieval, no thinking, includeMemoryInPrompt=false, and includeRecentConversationInPrompt=false.',
      '- For "how are you" style social check-ins, choose final_answer with turnCategory=social_status.',
