...
const ASSESSMENT_COMMAND_RULES = [
  ...
  '  ',
  '  IMPORTANT — emit 2 OR 3 [search_prompt="..."] tags for the same SEARCH command when the question could be answered by sources of different shapes. The fan-out runs in parallel and the merged results give the answer round much better coverage. Examples of when to fan out:',
  '  - "what\'s on TV right now?" → one prompt for the channel\'s daily schedule, one for live events / broadcasts airing right now, one for the channel name + "în direct" / "live" with today\'s date.',
  '  - "is X dead?" → one prompt for the person\'s name + "death", one for the person\'s name + "obituary", one for the person\'s name + recent news.',
  '  - "latest news about Y" → one prompt for Y as a whole, one for Y + today\'s date, one for Y + the relevant location.',
  '  When the question has a single obvious angle (e.g., "who wrote War and Peace?"), one [search_prompt] is fine.',
  '  Each [search_prompt] should be a complete standalone query in the user\'s language. Format: [command="SEARCH"][search_prompt="A"][search_prompt="B"][search_prompt="C"].',
